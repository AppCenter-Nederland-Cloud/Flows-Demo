export async function GetCalendarSuggestions(preferenceDate?: number) {
    ...
    const appointmentDuration = parseInt(process.env.CALENDAR_APPOINTMENT_DURATION || '0');
    const timeBetweenAppointments = parseInt(process.env.CALENDAR_TIME_BETWEEN || '0');
    ...
    const suggestionsRes = await fetch(`${digitalOceanUrl}/Calendar/sendSuggestions`, {
        method: "POST",
        body: JSON.stringify({
...
export async function CreateCalendarAppointment(start: string, end: string) {
    const digitalOceanUrl = process.env.DIGITAL_OCEAN_URL;
    const digitalOceanAuth = process.env.DIGITAL_OCEAN_AUTH;
    const communiqateApiKey = process.env.COMMUNIQATE_API_KEY;
    const communiqateOrganizationId = process.env.COMMUNIQATE_ORGANIZATION_ID;
