...
async function getEnergieLabel(adresseerbaarObjectIdentificatie: string) {
  ...
  const url =
    "https://public.ep-online.nl/api/v3/PandEnergielabel/AdresseerbaarObject";
  const options: any = {
    method: "GET",
    headers: {
  ...
  return await fetch(
    `${url}/${adresseerbaarObjectIdentificatie}`,
    options,
  ).then((a) => {
    return a.json();
  });
...
async function getBronhouder(url: string) {
  ...
  return await fetch(url, options).then((a) => {
    return a.json();
  });
}
export async function getBagInformation(query: string) {
  ...
  const objectData =
    bagData["_embedded"].adresseerbaarObject.verblijfsobject.verblijfsobject;

  const adresseerbaarObjectIdentificatie =
    bagData.adresseerbaarObjectIdentificatie;

  const straatnaam = bagData.openbareRuimteNaam;
  const huisnummer = bagData.huisletter
    ? `${bagData.huisnummer}${bagData.huisletter}`
    : `${bagData.huisnummer}`;
  const postcode = bagData.postcode;
  const woonplaats = bagData.woonplaatsNaam;
  const bouwjaar = pandData.oorspronkelijkBouwjaar;
  const oppervlakte = objectData.oppervlakte.toString();

  let gemeente = "Onbekend",
    provincie = "Onbekend";

  const bronhouderUrl =
    bagData["_embedded"].woonplaats["_links"].bronhouders[0].href;
  ...
  const woningtype =
    energieLabel && energieLabel.length > 0 && energieLabel[0].gebouwtype
      ? energieLabel[0].gebouwtype
      : "Onbekend";
  const label =
    energieLabel && energieLabel.length > 0 && energieLabel[0].labelLetter
      ? energieLabel[0].labelLetter
      : "Onbekend";
